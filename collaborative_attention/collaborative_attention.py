...
class CollaborativeAttention(nn.Module):
    def __init__(
        ...
        if dim_value_all % num_attention_heads != 0:
            raise ValueError("Value dimension ({}) should be divisible by number of heads ({})".format(dim_value_all, num_attention_heads))

        if not use_dense_layer and dim_value_all != dim_output:
            raise ValueError("Output dimension ({}) should be equal to value dimension ({}) if no dense layer is used".format(dim_output, dim_value_all))

        # save args
        self.dim_input = dim_input
        self.dim_value_all = dim_value_all
        self.value_dim_input = value_dim_input
        ...
        self.attention_head_size = (dim_key_query_all / num_attention_heads)  # does not have to be integer
        ...
        self.value = nn.Linear(value_dim_input, dim_value_all)
        ...
        if use_layer_norm:
            self.layer_norm = FakeBertLayerNorm(dim_value_all)
    ...
    def init_mixing_matrix(self, scale=0.2):
        ...
        if self.mixing_initialization is MixingMatrixInit.CONCATENATE:
            ...
            raise ValueError("Unknown mixing matrix initialization: {}".format(self.mixing_initialization))
    ...
    def forward(self, query_tensor, key_tensor, value_tensor, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, output_attentions=False,):
        ...
        from_sequence = query_tensor
        to_sequence = key_tensor
        ...
        # broadcast the shared key for all the heads (batch, 1, to_seq, dim)
        # mixed_key = key_layer[..., None, :, :]
        mixed_key = key_layer[..., None, :, :]* self.mixing[..., :, None, :]

        # (batch, head, from_seq, to_seq)
        attention_scores = torch.matmul(mixed_query, mixed_key.transpose(-1, -2))

        # add the content bias term (batch, to_seq, heads)
        ...
        value_layer = self.value(value_tensor)
